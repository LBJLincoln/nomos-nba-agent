...
### BEGIN Evolution Tuner addition (2026-03-20) ###
# --- Improvement: Island Model with migration to escape local optimum ---
# Type: config_change
# Description: Island model with 5 subpopulations (sizes 30,40,50,60,70) will maintain diversity and periodic migration of top 3 individuals every 10 generations will inject fresh solutions into each island, breaking stagnation.
# Hypothesis: Current single population is stagnating. Island model with periodic migration maintains diversity and breaks local optima.
# Params: {"population_strategy":"island_model","island_sizes":[30,40,50,60,70],"migration_interval":10,"migration_top_n":3,"tournament_k":7,"elitism":5}

from deap import tools, base
import random

def create_island_model(toolbox, island_sizes=[30,40,50,60,70], migration_interval=10, migration_top_n=3, tournament_k=7, elitism=5):
    """Create island model with periodic migration between subpopulations."""
    islands = []
    ...
    def evolve_island(island, toolbox, generations):
        """Evolve a single island."""
        for gen in range(generations):
            # Select and clone
            offspring = list(map(toolbox.clone, island['population']))
            ...
            island['population'] = toolbox.select(offspring, len(island['population']))
            
            # Update hall of fame
            island['halloffame'].update(island['population'])
            island['best'] = island['halloffame'][0]
            island['generation'] += 1
        
        return island
    
    def migrate(islands, top_n):
        """Migrate top individuals between islands."""
        migrants = []
        for island in islands:
            if island['best']:
                migrants.append(island['best'])
        
        migrants.sort(key=lambda x: x.fitness.values[0])
        top_migrants = migrants[:top_n]
        
        for island in islands:
            for migrant in top_migrants:
                island['population'].append(migrant)
                if len(island['population']) > max(island_sizes):
                    island['population'].sort(key=lambda x: x.fitness.values[0])
                    island['population'] = island['population'][:max(island_sizes)]
    
    def island_evolution_loop():
        """Main island evolution loop with periodic migration."""
        while True:
            # Evolve each island for migration_interval generations
            for island in islands:
                evolve_island(island, toolbox, migration_interval)
            
            # Perform migration
            migrate(islands, migration_top_n)
            
            # Log best from each island
            for i, island in enumerate(islands):
                if island['best']:
                    log(f"Island {i} best: {island['best'].fitness.values[0]}")
    
    return island_evolution_loop

# Replace standard evolution loop with island model
evolution_loop = create_island_model(
    toolbox,
